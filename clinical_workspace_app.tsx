...
const LAB_PRESETS = {
  lipid: {
    ...
    nextSteps: [
      ...
      "Review dietary dietary structure with a certified nutritionist.",
...
export default function App() {
  ...
  // Load a preset and reset chat context
  ...
  // Safe client-side Gemini call simulation or direct fetch integration
  const handleSendMessage = async (e) => {
    ...
    try {
      if (apiKey) {
        // Direct integration with standard Gemini 2.5/3.1 flash non-streaming call if API key is provided
        ...
        // Clinical response fallback generator to demonstrate workflow beautifully without requiring API keys
        setTimeout(() => {
  ...
  // Simulate parsing typed/pasted report raw data with Gemini or default parser fallback
  const handleParseCustomReport = async (e) => {
    ...
    try {
      if (apiKey) {
        // Direct extraction of medical structured JSON using Gemini
        ...
        // Mock success loader fallback for demo
        setTimeout(() => {
          const customReport = {
  ...
  // Utility to render the dynamic horizontal slider gauge for biomarkers
  const renderRangeGauge = (metric) => {
    const { value, minNormal, maxNormal, status, color } = metric;
    
    // Calculate safe boundaries for visual mapping
    const span = maxNormal - minNormal;
    const padding = span * 0.4; // visual range buffer
    const minVal = Math.max(0, minNormal - padding);
    const maxVal = maxNormal + padding;
    const totalRange = maxVal - minVal;
    
    // Convert value to percentage slider location
    let percent = ((value - minVal) / totalRange) * 100;
    percent = Math.max(2, Math.min(98, percent)); // keep indicator within bounds

    // Determine gauge bar visual classes
    ...
    return (
      <div className="space-y-2 bg-slate-900/50 border border-slate-800/80 p-4 rounded-xl hover:border-slate-700/60 transition-colors">
        ...
        <div className="relative pt-6 pb-2">
          ...
          {/* Slider track background colors */}
